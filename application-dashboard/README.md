...
Standalone job application tracker. Each application captures:

- Company, role, location, applied date, status
- **Source / channel** — LinkedIn, Referral, Company site, Recruiter, Other
- **Salary range** (min – max)
- **Notes**, **next-action**, **follow-up date** (overdue rows turn red)

Three views:

- **Applications** — fast add form + filterable table; click ▾ on a row to edit
  its notes, next-action, and follow-up date inline.
- **Time Tracker** — clock in/out with live elapsed, today / week / all-time
  totals, and a recent-sessions list.
- **Insights** — conversion funnel (Applied → Interview → Assessment → Offer
  with stage-to-stage %), salary KPIs, comp-goal indicator, pipeline
  breakdown, source breakdown ("what's working"), avg salary by status,
  and top salaries.
